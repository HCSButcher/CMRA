...
const Student = () => {
  ...
  const handleCourseViewDelete = (unitToDelete) => {
    setMaterials((prevMaterials) =>
      prevMaterials.filter((material) => material.unit !== unitToDelete)
    );
  };
  ...
  const commentDelete = (id) => {
    axios
      .delete(`https://project-2-1u71.onrender.com/comments/${id}`)
      .then(() => {
    ...
    const fetchSRegistrations = async () => {
      try {
        ...
        const response = await axios.get(
          `https://project-2-1u71.onrender.com/sRegistrations?email=${email}`,
          {
  ...
  useEffect(() => {
    const fetchMaterials = async () => {
      try {
        ...
        const response = await axios.get(
          'https://project-2-1u71.onrender.com/materials?recent=true',
          { headers }
        );
  ...
  useEffect(() => {
    const fetchAnnouncements = async () => {
      try {
        const token = localStorage.getItem('token');
        const response = await axios.get(
          'https://project-2-1u71.onrender.com/announcements',
          {
            headers: { Authorization: `Bearer ${token}` },
          }
        );
  ...
  useEffect(() => {
    const fetchComments = async () => {
      try {
        const token = localStorage.getItem('token');
        const response = await axios.get(
          'https://project-2-1u71.onrender.com/comments',
          {
            headers: { Authorization: `Bearer ${token}` },
          }
        );
    ...
    color: #444
      ...
      <div className={`side-menu ${isSidebarOpen ? 'open' : 'closed'}`}>
        ...
        <ul>
          ...
          <li>
            ...
            <a
              style={{ color: 'white' }}
              href="https://pastpapers.kabarak.ac.ke/login?next=%2F"
              target="blank"
              rel="noopener noreferrer"
            >
              {' '}
              <img src="http://localhost:3000/schools.png" alt="" /> &nbsp;{' '}
              <br /> <span>Past Papers</span>
            </a>{' '}
          </li>
          <li>
            {' '}
            <a
              style={{ color: 'white' }}
              href="https://eserver.kabarak.ac.ke/Students/"
              target="blank"
              rel="noopener noreferrer"
            >
              {' '}
              <img
                src="http://localhost:3000/teachers.png"
                alt=""
              /> &nbsp; <br /> <span>Student Portal</span>
            </a>{' '}
          </li>
          <li onClick={() => handleNavigate('/repository')}>
            {' '}
      ...
      <div className="container">
          ...
          <div className="cards-1">
            <div className="card-1">
              <div className="box-1">
                <h2>Comments</h2>
                <ul>
                  <li onClick={openCommentModal} className="btn">
                    Add
                  </li>
                </ul>
              </div>
          ...
          <div className="content-3">
            <div className="lecturer">
              <div className="titles">
                ...
                <ul>
                  <li onClick={openSCoursesModal} className="btn">
                    {' '}
                    Add
                  </li>
                </ul>
                  ...
                  {sRegistrations && sRegistrations.length > 0 ? (
                    sRegistrations.map((sRegistration) => (
                      <tr key={sRegistration._id}>
                        <td>
                          <ul>
                            <li className="btn" onClick={openViewUnitsModal}>
                              View units
                            </li>
                          </ul>
                        </td>
                    ...
                    materials.map((material) => (
                      <tr key={material._id}>
                        ...
                        <td>
                          <a
                            onClick={() =>
                              handleCourseViewDelete(material.unit)
                            }
                            className="btn"
                          >
                            {' '}
        ...
        {showSCoursesModal && (
          <div className="modal">
            <div className="modal-content">
              <span className="close" onClick={CloseSCoursesModal}>
                {' '}
                &times;
              </span>
            </div>
        ...
        {showViewUnitsModal && (
          <div className="modal">
            <div className="modal-content">
              <span className="close" onClick={CloseViewUnitsModal}>
                {' '}
                &times;
              </span>
              <ViewUnitsModal />
