...
const Lecturer = () => {
    ...
    const fetchCourses = async () => {
      try {
        ...
        const response = await axios.get(
          `https://project-2-1u71.onrender.com/courses?email=${email}`,
          {
    ...
    const fetchMaterials = async () => {
      try {
        ...
        const userResponse = await axios.get(
          'https://project-2-1u71.onrender.com/user',
          {
            headers: { Authorization: `Bearer ${token}` },
          }
        );

        const { role, email } = userResponse.data; // Assuming user data includes role and email
        ...
        if (Array.isArray(response.data)) {
          ...
          if (role === 'lecturer') {
            // Lecturer sees only their uploaded materials
            filteredMaterials = response.data.filter(
              (material) => material.lecturerEmail === email
            );
          } else if (role === 'admin' || role === 'student') {
            // Admins and Students see all materials
  ...
  useEffect(() => {
    const fetchUpdates = async () => {
      try {
        ...
        const response = await axios.get(
          `https://project-2-1u71.onrender.com/updates?email=${email}`,
          {
            headers: { Authorization: `Bearer ${token}` },
          }
        );
  ...
  useEffect(() => {
    const fetchAnnouncements = async () => {
      try {
        ...
        const response = await axios.get(
          `https://project-2-1u71.onrender.com/announcements?email=${email}`,
          {
            headers: { Authorization: `Bearer ${token}` },
          }
        );
  ...
  const handleDelete = (id) => {
    axios
      .delete(`https://project-2-1u71.onrender.com/announcements/${id}`)
      .then(() => {
        setAnnouncements(
  ...
  const updateDelete = (id) => {
    axios
      .delete(`https://project-2-1u71.onrender.com/updates/${id}`)
      .then(() => {
  ...
  const regDelete = (id) => {
    axios
      .delete(`https://project-2-1u71.onrender.com/courses/${id}`)
      .then(() => {
        setRegistrations(
  ...
  const uploadDelete = (id) => {
    axios
      .delete(`https://project-2-1u71.onrender.com/materials/${id}`)
      .then(() => {
  ...
  return (
    ...
    color: #444
      ...
      <div className={`side-menu ${isSidebarOpen ? 'open' : 'closed'}`}>
        ...
        <ul>
          <li onClick={() => handleNavigate('/enroll')}>
            {' '}
            <img src="http://localhost:3000/teachers.png" alt="" /> &nbsp;{' '}
            <span>Course Management</span>
          </li>
          <li onClick={openComment2Modal}>
            {' '}
          ...
          <li>
            ...
            <a
              style={{ color: 'white' }}
              href="https://kabarak.ac.ke/library"
              target="_blank"
              rel="noopener noreferrer"
            >
              {' '}
              <img src="http://localhost:3000/schools.png" alt="" /> &nbsp;
              <br /> <span>Library</span>
            </a>
          </li>
          <li onClick={openUploadModal}>
            {' '}
            <img src="http://localhost:3000/download.png" alt="" /> &nbsp;
      ...
      <div className="container">
          ...
          <div className="cards-5">
            <div className="card-5">
              <div className="box-5">
                ...
                <ul>
                  <li onClick={openUpdateModal} className="btn">
                    {' '}
                    Add
                  </li>
                </ul>
                  ...
                  {updates && updates.length > 0 ? (
                    updates.map((update) => (
                      <tr key={update._id}>
                        ...
                        <td>
                          {' '}
                          <li className="btn" onClick={openUploadModal}>
                            {' '}
                            Send
                          </li>
                          <li
                            className="btn"
                            onClick={() => updateDelete(update._id)}
                          >
                            {' '}
                            Delete
                          </li>
            ...
            <div className="cards-6">
              <div className="box-6">
                <h2> Announcements</h2>
                <ul>
                  <li className="btn" onClick={openAnnouncementModal}>
                    Add
                  </li>
                </ul>
              </div>
          ...
          <div className="content-6">
            <div className="lecturer-5">
              <div className="titles-5">
                ...
                <ul>
                  <li className="btn" onClick={() => navigate('/courses')}>
                    {' '}
                    Add
                  </li>
                </ul>
                      ...
                      <tr key={registration._id}>
                        <td>
                          <ul>
                            <li
                              className="btn"
                              onClick={() => regDelete(registration._id)}
                            >
                              {' '}
                              Delete
                            </li>
                        ...
                        <td>
                          {new Date(registration.regDate).toLocaleDateString()}{' '}
                        </td>
                      </tr>
                    ))
                  ) : (
            ...
            <div className="new-student-5">
              <div className="titles-5">
                ...
                <ul>
                  <li className="btn" onClick={openViewModal}>
                    View All
                  </li>
                </ul>
              </div>
                      ...
                      <tr key={material._id}>
                        ...
                        <td>
                          <ul>
                            <li
                              className="btn"
                              onClick={() => uploadDelete(material._id)}
                            >
                              {' '}
                              Delete
                            </li>
        ...
        {showCoursesModal && (
          <div className="modal">
            <div className="modal-content">
              <span className="close" onClick={CloseCoursesModal}>
                &times;{' '}
              </span>
        ...
        {showAnnouncementModal && (
          <div className="modal">
            <div className="modal-content">
              <span className="close" onClick={CloseAnnouncementModal}>
                &times;
              </span>
        ...
        {showComment2Modal && (
          <div className="modal">
            <div className="modal-content">
              <span className="close" onClick={CloseComment2Modal}>
                &times;{' '}
              </span>
